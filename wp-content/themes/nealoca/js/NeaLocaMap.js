...
$( document ).ready( function() 
         {
             // default
             var nlm = new NeaLocaMap();
             nlm.centrerRegion();
             // emplacement
             jQuery( '#nlmEmplacement' ).click(function( event ) 
                 {
                     event.preventDefault();
                     nlm.centrerVillage();
                 });
             // acces
             jQuery( '#nlmAcces' ).click( function( event ) 
                 {
                     // defaut thessa 
                     event.preventDefault();
                     nlm.distanceEtCalcule('thessa');
                     jQuery( '#nlmAccesThessa' ).click( function( event)
                         {
                             nlm.distanceEtCalcule('thessa');
                         });
                     jQuery( '#nlmAccesNeamoudania' ).click( function( event)
                         {
                             nlm.distanceEtCalcule('neamoudania');
                         });
                     jQuery( '#nlmAccesSerres' ).click( function( event)
                         {
                             nlm.distanceEtCalcule('serres');
                         });
                     jQuery( '#nlmAccesBxl' ).click( function( event)
                         {
                             nlm.distanceEtCalcule('bxl');
                         });
                 });
             // region
             jQuery( '.nlmRegion' ).click( function( event ) 
                 {
                     event.preventDefault();
                     nlm.centrerRegion();
                     nlm.chalkidiki();
                     ...
                     // ville
                     jQuery( '#nlmVilles' ).click( function( event )
                         {
                             event.preventDefault();
                             nlm.centrerVilles();
                         });
             ...
             // village
             jQuery( '#nlmVillage' ).click( function( event ) 
                 {
                    event.preventDefault();
                    nlm.centrerVillage();
                    
                    // recuperer les infos 
                    var cis_json = jQuery( '#nlmCisJson' ).val();
                    if( cis_json !== undefined )
                    {
                        var cisParCategories = jQuery.parseJSON( cis_json );
                    }

                    for( var nomCategorie in cisParCategories )
                    {
                        var idCategorie = '#nlm' + nomCategorie;
                        var cisParCategorie = cisParCategories[nomCategorie];
                        jQuery( idCategorie ).click( { cisParCategorie : cisParCategorie , nlm : nlm } , function( event )
                        {
                            event.preventDefault();
                            nlm.nettoyage();
                            for( var i = 0; i < event.data.cisParCategorie.length; i++ )
                            {
                                var ci = event.data.cisParCategorie[i];
                                var loc = new google.maps.LatLng( parseFloat( ci.latitude ), parseFloat( ci.longitude ) );
                                var mq = { localisation : loc, titre : ci.categorie, anime : 'true' }; // FIXME image : ci.urlicon pour avoir les icons choisi par l'user 
                                nlm.marqueur( mq );
                            }
                        });
                    }
...
// utiles
//google.maps.event.addDomListener(window, 'load', initNeaLocaMap);
var map;
function initNeaLocaMap()
{
   var nlm = new NeaLocaMap();
   nlm.centrerRegion();
   //nlm.marqueur({ titre : 'ok', anime : 'true', texte : 'montexte' });
   //nlm.chalkidiki( nlm.getCoordonneesChalkidiki() );
   //nlm.marqueur({ localisation: '', titre : '', anime : '', texte : '', image : '' });
 //  nlm.distance();
   //nlm.calculeDistance();
} 
